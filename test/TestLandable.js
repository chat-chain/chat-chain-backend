...
async function execc (){
  ...
  await sendFreeTest (web3,accounts[1],process.env.E_WALLET_PKEY_FREE_USER, accProx,accounts[3],process.env.E_WALLET_PKEY_SLAVE_PROXY,eveeContract,recipiantContract,MaxFee,comID,4)


  
	}

    async function sendFreeTest (web3,free_account,free_accoun_pkey,master_proxy,slave_proxy,slave_proxy_pkey,eveeContract,TestContract,MaxFee,comID,num){
    
        // test deadline in future
        const deadline = Date.now() + 100000
    
        const TXtoRecipiant = await TestContract.methods
          .land(await TestContract.methods
